...
def setup_database(conn):
    with conn.cursor() as cur:
        ...
        """)
        # Add an index on filename for faster deletions
        print("Creating index on 'filename' column if it doesn't exist...")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_filename ON documents (filename);")
...
# --- NEW: Function to clear old data by filename ---
def clear_existing_data_for_file(conn, filename: str):
    """Deletes all entries for a specific filename to prevent duplicates and allow for updates."""
    print(f"Checking for and clearing any existing data for '{filename}'...")
    with conn.cursor() as cur:
...
# --- Part 5: Main Execution Block (MODIFIED to include deletion) ---
def main():
    parser = argparse.ArgumentParser(description="Index or re-index a document (PDF or DOCX) into the database.")
    ...
    try:
        ...
        # Get the simple filename (e.g., 'sample.pdf')
        filename_to_index = os.path.basename(args.file)
        
        # --- NEW: Call the function to clear old data for this file ---
        clear_existing_data_for_file(connection, filename_to_index)
        
        raw_text = extract_text_from_file(args.file)
        if not raw_text.strip():
            print("Warning: The extracted text is empty. No new data will be indexed.")
            return
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            if embedding:
                ...
                data_to_insert.append((filename_to_index, clean_chunk, embedding, split_strategy))
        ...
        print(f"\n--- Document '{filename_to_index}' successfully indexed! ---")
